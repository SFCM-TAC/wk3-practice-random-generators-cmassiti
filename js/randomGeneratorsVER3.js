...
var slotMachineGenerator = new Generator(['Parsnips', 'BANKRUPT', 'Manilla Wafers', 'Try Again?', '777', 'Cherries!']);
...
function handleSlotMachineShake() {
  const resultElement = document.getElementById('slotMachineResult');
  const result = slotMachineGenerator.getValue();
  resultElement.innerHTML = result;
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (slotMachineButton) {
    slotMachineButton.addEventListener('click', handleSlotMachineShake);
  }
});
